 ...
 *
 * -----------------------------------------------------------------
 * 2019/7/11 10:44 : Create ArcProgressBar.java (JoDragon);
 * -----------------------------------------------------------------
 */
public class ArcProgressBar extends ProgressBar {
    ...
    private ArcProgressBar.OnCenterDraw mOnCenter;
    ...
    public void setOnCenterDraw(ArcProgressBar.OnCenterDraw mOnCenter) {
    ...
    protected synchronized void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if(widthMode!=MeasureSpec.EXACTLY){
            int widthSize = (int) (mRadius*2+mBoardWidth*2+getPaddingLeft()+getPaddingRight());
            widthMeasureSpec = MeasureSpec.makeMeasureSpec(widthSize,MeasureSpec.EXACTLY);
        }
        if(heightMode != MeasureSpec.EXACTLY){
            int heightSize = (int) (mRadius*2+mBoardWidth*2+getPaddingTop() + getPaddingBottom());
    ...
    protected synchronized void onDraw(Canvas canvas) {
        ...
        if(mStylePogress == STYLE_ARC){
            ...
            canvas.drawArc(mArcRectf,0,360,false,mArcPaint);
            //绘制完成部分
            mArcPaint.setColor(mProgressColor);
            canvas.drawArc(mArcRectf,270+angle,targetmDegree,false,mArcPaint);
        }else{
            if(mBgShow)
